...
PATRONES = [
    ...
    (
        ...
        re.compile(
            ...
            r"expediente|folio)\s*(?:(?:n[oº°]|nro|num(?:ero)?)\.?\s*)?[:#\-]?\s*(?P<val>[A-Z]{0,3}[\d][\d.\-]{3,15})",
            re.I,
        ),
        Confianza.ALTA,
    ),
    ...
    (
        ...
        re.compile(r"\b(?:firmado\s+(?:digitalmente\s+)?por|firma\s+electr[oó]nica|"
                   r"registro\s+m[eé]dico|r\.?m\.?\s*n[oº°.]?|matr[ií]cula\s+profesional|"
                   r"colegiado)\s*[:#-]?\s*(?P<val>[^\n;|]{2,60})", re.I),
        Confianza.ALTA,
    ),
]
